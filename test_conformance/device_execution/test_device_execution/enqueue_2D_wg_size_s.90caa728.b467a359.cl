...
void block_fn(int level, int maxGlobalWorkSize, __global int* rnd, __global int* res)
{
  ...
  if((tidX < (get_global_size(0) >> 1)) && ((tidY < (get_global_size(1) >> 1)) || get_global_size(1) == 1))
  {
...
kernel void enqueue_2D_wg_size_some_eq(__global int* res, int level, int maxGlobalWorkSize, __global int* rnd)
{
