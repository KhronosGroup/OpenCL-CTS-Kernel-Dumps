...
kernel void helper_ndrange_3d_glo(__global int* res, uint n, uint len, __global uint* glob_size_arr, __global uint* loc_size_arr, __global int* val,  __global uint* ofs_arr)
{
  ...
  for(int i = 0; i < n; i++)
  {
    ...
    if (global_work_size <= (len * len))
    {
      ...
      ndrange_t ndrange = ndrange_3D(glob_size);
