...
kernel void helper_ndrange_2d_loc(__global int* res, uint n, uint len, __global uint* glob_size_arr, __global uint* loc_size_arr, __global int* val,  __global uint* ofs_arr)
{
  ...
  for(int k = 0; k < n; k++)
  {
    for(int i = 0; i < n; i++)
    {
      if (glob_size_arr[(i + 1) % n] >= loc_size_arr[k])
      {
        ...
        ndrange_t ndrange = ndrange_2D(glob_size, loc_size);
