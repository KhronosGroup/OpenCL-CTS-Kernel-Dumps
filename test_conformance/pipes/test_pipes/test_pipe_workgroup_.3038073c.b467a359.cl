        ...
        __kernel void test_pipe_workgroup_write_float2(__global float2 *src, __write_only pipe float2 out_pipe)
        {
        ...
        __kernel void test_pipe_workgroup_read_float2(__read_only pipe float2 in_pipe, __global float2 *dst)
        {
            int gid = get_global_id(0);
            __local reserve_id_t res_id;
